define([
	"dojo/base/array", // array.forEach array.indexOf array.some
	"dojo/base/config", // config
	"dojo/base/declare", // declare
	"dojo/base/Deferred", // Deferred
	"dojo/dom", // dom.byId
	...
	"dojo/base/lang", // lang.clone lang.hitch lang.isArray lang.isFunction lang.isString lang.trim
	"dojo/on", // on()
	"dojo/query", // query
	"dojo/domReady",
	"dojo/sniff", // has("ie") has("mozilla") has("opera") has("safari") has("webkit")
	"dojo/topic", // topic.publish() (publish)
	"dojo/base/unload", // unload
	"dojo/base/url", // url
	...
	"../main"    // dijit._scopeName
			...
			Widget, CssStateMixin, selectionapi, rangeapi, htmlapi, focus, dijit){
	...
	// the id==dijit._scopeName + ".editor.RichText.value" (typically "dijit/editor/RichText.value). For example,
	// something like this will work:
	//
	//	<textarea id="dijit.editor.RichText.value" style="display:none;position:absolute;top:-100px;left:-100px;height:3px;width:3px;overflow:hidden;"></textarea>

	var RichText = declare("dijit.editor.RichText", [Widget, CssStateMixin], {
		// summary:
		//		dijit/editor/RichText is the core of dijit.Editor, which provides basic
		...
		postCreate: function(){
			if("textarea" === this.domNode.tagName.toLowerCase()){
				console.warn("RichText should not be used with the TEXTAREA tag.  See dijit.editor.RichText docs.");
			}
			...
			topic.publish(dijit._scopeName + ".editor.RichText::init", this);
		},
		...
		_localizeEditorCommands: function(){
			...
			// tags:
			//		private
			if(RichText.editorCommandsLocalized){
			...
			RichText.editorCommandsLocalized = true;
		...
		open: function(/*DomNode?*/ element){
			...
			topic.publish(dijit._scopeName + ".editor.RichText::open", this);
			...
			if(this.name !== "" && (!config["useXDomain"] || config["allowXdRichTextSave"])){
				var saveTextarea = dom.byId(dijit._scopeName + ".editor.RichText.value");
		...
		addStyleSheet: function(/*dojo/base/url*/ uri){
			// summary:
...
//			console.debug("dijit/editor/RichText.addStyleSheet(): Style sheet "+url+" is already applied");
				return;
			}
		...
		removeStyleSheet: function(/*dojo/base/url*/ uri){
			...
			if(index === -1){
//			console.debug("dijit/editor/RichText.removeStyleSheet(): Style sheet "+url+" has not been applied");
				return;
			}
...
//				}
		...
		getNodeHtml: function(/* DomNode */ node){
			// summary:
			//		Deprecated.   Use dijit/editor/html::_getNodeHtml() instead.
			// tags:
			//		deprecated
			kernel.deprecated('dijit.Editor::getNodeHtml is deprecated', 'use dijit/editor/html::getNodeHtml instead', 2);
			return htmlapi.getNodeHtml(node); // String
		},

		getNodeChildrenHtml: function(/* DomNode */ dom){
			// summary:
			//		Deprecated.   Use dijit/editor/html::getChildrenHtml() instead.
			// tags:
			//		deprecated
			kernel.deprecated('dijit.Editor::getNodeChildrenHtml is deprecated', 'use dijit/editor/html::getChildrenHtml instead', 2);
