define(["dojo", "doh", "../../../base/sniff", "require"], function(dojo, doh, has, require){
	doh.register("tests.base._loader.bootstrap", [
		...
		function getText(t){
			if(require.getText){
				var text = require.getText(require.toUrl("dojo/tests/base/loader/getText.txt")).replace(/\r|\n/g, "");
				t.assertEqual("dojo._getText() test data", text);
				if(dojo._getText){
					text = dojo._getText(require.toUrl("dojo/tests/base/loader/getText.txt")).replace(/\r|\n/g, "");
					t.assertEqual("dojo._getText() test data", text);
				}
			}
		},
