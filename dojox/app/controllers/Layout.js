define(["dojo/base/declare", "dojo/base/lang", "dojo/base/array", "dojo/base/window",
...
function(declare, lang, array, win, query, domGeom, domAttr, domStyle, registry, LayoutBase, layout, constraints){
	...
	return declare("dojox.app.controllers.Layout", LayoutBase, {
		...
		_doResize: function(view){
			...
			var mb = {};
			if( !("h" in mb) || !("w" in mb) ){
				mb = lang.mixin(domGeom.getMarginBox(node), mb);	// just use dojo/base/html.marginBox() to fill in missing values
			}

			// Compute and save the size of my border box and content box
			// (w/out calling dojo/base/html.contentBox() since that may fail if size was recently set)
			if(view !== this.app){
