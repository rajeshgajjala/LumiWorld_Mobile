...
var DnD = declare("dojox.grid.enhanced.plugins.DnD", _Plugin, {
	...
	constructor: function(grid, args){
		...
		this.Container = query(".dojoxGridMasterView", this.grid.domNode)[0];
		this._initEvents();
	},
	destroy: function(){
		this.inherited(arguments);
	...
	_createDnDUI: function(evt, isMovingIn){
		...
		html.style(this.Container, "height", viewPos.h + "px");
		try{
	...
	_markTargetAnchor: function(evt){
		...
		var height, width, left, top,
			...
			pos = html.position(this.Container);
		if(!targetAnchor){
			...
			this.Container.appendChild(targetAnchor);
		}
		switch(t){
			case "col":
				height = pos.h;
