define([
	...
	"dojox/mvc/at",
	"dojox/mvc/equals"
], function(doh, Stateful, WidgetBase, at, equals){
	doh.register("dojox.mvc.tests.doh.atEquals", [
		function single(){
			var a = [0, 1, 2],
			 model = new Stateful({value: a}),
			 w = new WidgetBase({value: at(model, "value").equals(equals)});
			w.startup();
		...
		function wildcard(){
			var a = [0, 1, 2],
			 model = new Stateful({value: a}),
			 w = new WidgetBase({_getPropertiesAttr: function(){ return ["value"]; }, "*": at(model, "*").equals(equals)});
			w.startup();
			model.set("value", a.slice());
			doh.is(a, w.value, "The widget should keep the original value");
			w.set("value", at(model, "value"));
