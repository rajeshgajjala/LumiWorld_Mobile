define([
...
], function(doh, array, config, declare, WidgetBase, at, TemplatedMixin, WidgetsInTemplateMixin, Container, _TextBox, getStateful, WidgetList, template, childTemplate, childBindings){
	var a = getStateful([
		{
			Serial: "A111",
			First: "Anne",
	...
	doh.register("dojox.mvc.tests.doh.WidgetList", [
		...
		function objectInChildType(){
			...
			var w = new (declare([WidgetBase, TemplatedMixin, WidgetsInTemplateMixin], {templateString: template}))({childData: data});
			w.startup();
			...
			doh.t(array.every(simpleChildrenWithRegularDijitInMixin, function(child){ return child.isMyWidget && child.isMyMixin && child.addChild && !child._setValueAttr; }), "simpleChildrenWithRegularDijitInMixin should be created by My.Widget, My.Mixin and dijit/Container");
		}
	]);
});
