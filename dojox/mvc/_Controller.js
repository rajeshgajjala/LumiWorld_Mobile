define([
	...
	return declare("dojox.mvc._Controller", [Stateful, _atBindingMixin], {
		postscript: function(/*Object?*/ params, /*DomNode|String?*/ srcNodeRef){
			...
			// If there is dijit/WidgetBase in upper class hierarchy (happens when this descendant is mixed into a widget), let WidgetBase do all work
		...
		set: function(/*String*/ name, /*Anything*/ value){
			...
			if(!this._applyAttributes){
				if((value || {}).atsignature == "dojox.mvc.at"){
					...
					// Otherwise align the setter interface to WidgetBase
		...
		_set: function(/*String*/ name, /*Anything*/ value){
			// summary:
			//		Implement _set() interface so that _set() behavior is consistent whether the instance inherits WidgetBase or not.
			//		If the instance does not inherit WidgetBase, use dojo/Stateful/_changeAttrValue() that's equivalent to dijit/WidgetBase._set().
			// name: String
			//		The property name.
			// value: Anything
			//		The property value.
			...
			if(!this._applyAttributes){
				// Call dojo/Stateful/_changeAttrValue() that's equivalent to dijit/WidgetBase/_set()
