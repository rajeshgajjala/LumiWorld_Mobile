define([
	...
	"dijit/WidgetBase",
	"dojo/regexp"
], function(declare, lang, when, WidgetBase, regexp){

	return declare("dojox.mvc.Container", WidgetBase, {
		...
		// ContainedWidgets: [protected] dijit/_Widget[]
		...
		_createBody: function(){
			...
			if(this._parser){
				return when(this._parser.parse(this.srcNodeRef,{
					...
					_self.ContainedWidgets = widgets;
				});
			}
		},
		...
		_destroyBody: function(){
			...
			// tags:
			//		protected
			if(this.ContainedWidgets && this.ContainedWidgets.length > 0){
				for(var n = this.ContainedWidgets.length - 1; n > -1; n--){
					var w = this.ContainedWidgets[n];
